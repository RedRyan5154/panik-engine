...
class Window():
    ...
    def update(self, uimanager=None, ui=None):
        ...
        for el in self.queue:
            if el.parent:
                if not el.parent.hide:
                    if el.type == "element":
                        self.WIN.blit(el.image, (el.x-el.size_x/2+el.parent.x, el.y-el.size_y/2+el.parent.y))
                    if el.type == "entity":
                        self.WIN.blit(el.image, (el.x-el.size_x/2+el.parent.x, el.y-el.size_y/2+el.parent.y))
                        el.colision.x = el.x+el.colisionoffsetx-el.colisionsizex/2+el.parent.x
                        el.colision.y = el.y+el.colisionoffsety-el.colisionsizey/2+el.parent.y
                ...
                if el.type == "element":
                    self.WIN.blit(el.image, (el.x-el.size_x/2, el.y-el.size_y/2))
                if el.type == "entity":
                    self.WIN.blit(el.image, (el.x-el.size_x/2, el.y-el.size_y/2))
                    if el.showcolision and self.devmode:
                        font = pygame.font.Font(None, 30)
        ...
        if self.showfps:
            ...
            text = font.render("FPS: " + str(round(self.clock.get_fps())), True, (0, 0, 0))
...
class Events():
    def __init__(self):
        ...
        self.SELECTION_LIST_REMOVE_SELECTION = pygame_gui.UI_SELECTION_LIST_DROPPED_SELECTION
        self.SELECTION_LIST_DOUBLE_SELECT = pygame_gui.UI_SELECTION_LIST_DOUBLE_CLICKED_SELECTION
...
class Element():
    def __init__(self, id, image, x, y, image_scale=100, rotation=0, flip=[False, False], parent=False):
        self.id = id
        self.animationidx = 0
        ...
        else: self.image = image.image
        self.size_x = self.image_scale*self.image.get_width()/100
        self.size_y = self.image_scale*self.image.get_height()/100
        self.image = pygame.transform.flip(pygame.transform.rotate(pygame.transform.scale(self.image, (self.size_x, self.size_y)), self.rotation), self.flip[0], self.flip[1])

    def animate(self, animation, animname=None, delay=0.1):
        if animname:
            self.image = animation.animations[animname]  
            self.size_x, self.size_y = self.image_scale*self.image.get_width()/100, self.image_scale*self.image.get_height()/100
            self.image = pygame.transform.flip(pygame.transform.rotate(pygame.transform.scale(self.image, (self.size_x, self.size_y)), self.rotation), self.flip[0], self.flip[1])
        else:
...
class Entity():
    def __init__(self, id, image, x, y, image_scale=100, rotation=0, flip=[False, False], parent=False, colisionoffsetx=0, colisionoffsety=0, colisionsizex=100, colisionsizey=100):
        self.id = id
        self.animationidx = 0
        self.starttime = time.time()
        self.parent = parent
        ...
        self.colision = pygame.Rect(self.x+colisionoffsetx-colisionsizex/2, self.y+colisionoffsety-colisionsizey/2, colisionsizex, colisionsizey)
        self.image = pygame.transform.flip(pygame.transform.rotate(pygame.transform.scale(self.image, (self.size_x, self.size_y)), self.rotation), self.flip[0], self.flip[1])
    ...
    def updatePosition(self):
        self.colision.x, self.colision.y = self.x-self.size_x/2+self.parent.x, self.y-self.size_y/2+self.parent.y
...
class Colision():
    def __init__(self, id, colisionposx=0, colisionposy=0, colisionsizex=100, colisionsizey=100, parent=False):
        self.id = id
        self.x = colisionposx
        self.y = colisionposy
        self.colisionsizex = colisionsizex
        ...
        self.type = "colision"
        self.showcolision = True
        self.colision = pygame.Rect(colisionposx-colisionsizex/2, colisionposy-colisionsizey/2, colisionsizex, colisionsizey)
...
class UIButton():
    def __init__(self, text, manager, x, y, sizex=100, sizey=50):
        ...
        self.button = pygame_gui.elements.UIButton(relative_rect=self.rect, text=self.text, manager=self.manager.manager, allow_double_clicks=True)
...
class UIText():
    def __init__(self, text, manager, x, y, sizex=200, sizey=100):
        ...
        self.x, self.y, self.sizex, self.sizey = x, y, sizex,sizey
        self.rect = pygame.Rect((self.x, self.y), (self.sizex, self.sizey))
        self.text = pygame_gui.elements.UITextBox(self.text, self.rect, self.manager.manager)
...
class UILabel():
    def __init__(self, text, manager, x, y, sizex=400, sizey=100):
        ...
        self.x, self.y, self.sizex, self.sizey = x, y, sizex,sizey
        self.rect = pygame.Rect((self.x, self.y), (self.sizex, self.sizey))
        self.text = pygame_gui.elements.UILabel(self.rect, self.text, self.manager.manager)
...
class UITextEntry():
    def __init__(self, text, manager, x, y, sizex=200, sizey=50):
        ...
        self.textentry = pygame_gui.elements.UITextEntryLine(self.rect, self.manager.manager)
...
class UIDropDownMenu():
    def __init__(self, options, default_option, manager, x, y, sizex=100, sizey=25):
        ...
        self.x, self.y, self.sizex, self.sizey = x, y, sizex,sizey
        self.rect = pygame.Rect((self.x, self.y), (self.sizex, self.sizey))
        self.dropdownmenu = pygame_gui.elements.UIDropDownMenu(self.options, self.default_option, self.rect, self.manager.manager)
...
class UIHorizontalSlider():
    def __init__(self, manager, x, y, sizex=200, sizey=25, starting_value=0, range=(0, 100)):
        self.range = range
        self.starting_value = starting_value
        self.manager = manager
        self.x, self.y, self.sizex, self.sizey = x, y, sizex,sizey
        self.rect = pygame.Rect((self.x, self.y), (self.sizex, self.sizey))
        self.slider = pygame_gui.elements.UIHorizontalSlider(self.rect, self.starting_value, self.range, self.manager.manager)
...
class UISelectionList():
    def __init__(self, options, manager, x, y, sizex=200, sizey=200):
        ...
        self.x, self.y, self.sizex, self.sizey = x, y, sizex,sizey
        self.rect = pygame.Rect((self.x, self.y), (self.sizex, self.sizey))
        self.selectionlist = pygame_gui.elements.UISelectionList(self.rect, self.options, self.manager.manager)
...
class UICheckList():
    def __init__(self, options, manager, x, y, sizex=200, sizey=200):
        ...
        self.x, self.y, self.sizex, self.sizey = x, y, sizex,sizey
        self.rect = pygame.Rect((self.x, self.y), (self.sizex, self.sizey))
        self.selectionlist = pygame_gui.elements.UISelectionList(self.rect, self.options, self.manager.manager, allow_multi_select=True, allow_double_clicks=False)
...
class UIConfirmationDialog():
    def __init__(self, title, text, manager, x, y, sizex=260, sizey=200, priority=True):
        ...
        self.x, self.y, self.sizex, self.sizey = x, y, sizex,sizey
        self.rect = pygame.Rect((self.x, self.y), (self.sizex, self.sizey))
        self.confirmdialog = pygame_gui.windows.UIConfirmationDialog(self.rect, self.manager.manager, self.text, window_title=self.title, blocking=self.priority)
